...
## ```~~~ English -> ```
Stock market notification management system for users (lambda functions)

- decoupling of different (classes / lambda functions) for scalability
- flexibility to add and remove observers at runtime
- reusable by being in separate components (lambda functions or classes)

 
The system notifies the user whenever a stock has a price change.

Notifications can be sent via Email, Console, or SMS, according to the options selected by the user.
